...
parser.add_argument('--test_area', default=5, type=str, help='Predicted sequence id [default: 08]')
...
class Tester:
    def __init__(self):
        ...
        test_dataset = s3distest('test', test_area=FLAGS.test_area, batch_size=FLAGS.batch_size)
    ...
    def merge_and_store(self):
        ...
        root_dir = os.path.join(FLAGS.result_dir, str(self.test_dataset.test_area), 'predictions')
        ...
        for j in tqdm(range(N)):
            if FLAGS.infer_type == 'all':
                ...
                proj_file = Path(self.test_dataset.dataset_path) / '{:s}_proj.pkl'.format(proj_file_name)
